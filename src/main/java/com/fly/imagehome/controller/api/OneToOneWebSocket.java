...
/**
 * @Classname OneToOneWebSocket
 * @Description OneToOneWebSocket
 * @Date 2022/1/7 11:11
 * @Author Fly
 * @Version 1.0
 */
...
public class OneToOneWebSocket {
    ...
    public void onMessage(String message, Session session) {
        log.info("服务端收到[{}]的消息[{}]", session.getUserPrincipal().getName(), message);
        try {
            MyMessage myMessage = DozerConvertorUtil.convertor(message, MyMessage.class);
            if (myMessage != null) {
    ...
    private void sendMessage(String message, Session toSession) {
        try {
            log.info("[{}]给服务端发送消息[{}]", toSession.getUserPrincipal().getName(), message);
