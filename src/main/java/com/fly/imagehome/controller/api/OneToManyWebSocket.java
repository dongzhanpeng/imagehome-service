...
/**
 * @Classname OneToManyWebSocket
 * @Description OneToManyWebSocket
 * @Date 2022/1/7 11:07
 * @Author Fly
 * @Version 1.0
 */
...
public class OneToManyWebSocket {
    ...
    public void onOpen(Session session) {
        ...
        clients.put(session.getUserPrincipal().getName(), session);
        log.info(userName + ":加入，当前在线人数为：{}", onlineCount.get());
    }
    ...
    public void onMessage(String message, Session session) {
        ...
        String msg = userName.concat("：").concat(message);
    ...
    private void sendMessage(String message, Session fromSession) {
        for (Map.Entry<String, Session> sessionEntry : clients.entrySet()) {
            ...
            if (!fromSession.getId().equals(toSession.getId())) {
                String userName = fromSession.getUserPrincipal().getName();
